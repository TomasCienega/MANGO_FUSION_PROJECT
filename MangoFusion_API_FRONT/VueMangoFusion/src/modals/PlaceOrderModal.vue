  ...
  >
    ...
    >
      ...
      <div v-if="errorList.length > 0" class="alert alert-danger" role="alert">
        <span class="d-block" v-for="error in errorList" :key="error">
          {{ error }}
        </span>
      </div>
...
const submitOrder = async () => {
  try {
    ...
    // 1. Validaciones
    if (!orderData.pickUpName) errorList.push('Name is required.');
    if (!orderData.pickUpPhoneNumber) errorList.push('Phone number is required.');
    if (!orderData.pickUpEmail) errorList.push('Email is required.');
    if (orderData.pickUpEmail && !emailPattern.test(orderData.pickUpEmail)) {
