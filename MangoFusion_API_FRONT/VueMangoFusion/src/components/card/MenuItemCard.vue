  ...
  <div class="col-12 col-sm-6 col-lg-4">
    <div class="card h-100 border-0 shadow-sm rounded-4">
      <div class="position-relative">
        <img
          :src="CONFIG_IMAGE_URL + props.menuItem.image"
      ...
      <div class="card-body p-3 p-sm-4">
        ...
        <div class="d-flex justify-content-between align-items-start mb-2">
          <h5 class="card-title mb-0 small fw-semibold">{{ props.menuItem.name }}</h5>
          <div class="d-flex align-items-center ms-2">
            <span class="fw-semibold text-success">{{ props.menuItem.price.toFixed(2) }}</span>
          </div>
        </div>
        ...
        <div class="d-flex flex-wrap align-items-center gap-2 mb-3">
          <div
            ...
            v-if="props.menuItem.specialTag"
          >
            <i class="bi bi-star"></i>&nbsp;
            <span class="small text-body-secondary"> {{ props.menuItem.specialTag }}</span>
          </div>
...
const itemQuantity = computed(() => (cartItem.value ? cartItem.value.quantity : 0));
...
const increaseQuantity = () => {
  if (cartItem.value) {
    cartStore.updateQuantity(cartItem.value.id, cartItem.value.quantity + 1);
  }
};
